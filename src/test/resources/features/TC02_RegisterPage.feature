...
Feature: Functional testing for DSAlgo Register page
  ...
  Scenario Outline: Invalid register attempts for "<scenarioType>"
    ...
    Examples:
      ...
      | Null value in  confirm password for register          | submits the register form | Please fill out this field.                             |
      | with specialcharacter password for register           | submits the register form | Password does not match requirement                     |
      | with password less then eight characters for register | submits the register form | Password does not match requirement                     |
      | with mismatch password for register                   | submits the register form | password_mismatch:The two password fields didn’t match. |
      | with specialcharacter username for register           | submits the register form | Username must be alphanumeric                           |
